...
**Custom Properties on Cards**: The Huly API natively stores custom component fields (e.g., custom6a8...) on the `Card` object directly. The `get_card` implementation (`src/huly/operations/cards.ts`) intentionally collects these non-standard fields and exposes them in a `properties` object. When updating the API client or modifying `get_card`, ensure this extraction logic is preserved so agents can read custom Card Component fields.
